...
"""
        ...
        messagebox.showerror("Error", f"Sorry, failed to fetch weather data.\n{e}")
...
def show_history():
    try:
        with open(os.path.join(BASE_DIR, "weather_log.txt"), "r", encoding="utf-8") as file:
            history = file.read()
        if not history.strip():
            history = "No saved weather history."
        result_text.config(state="normal")
